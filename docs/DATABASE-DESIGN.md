...
- Enforcement points: reads and writes are scoped by `organization_id` in the service layer, not at the persistence layer, so the same store runs on JPA (PostgreSQL) and in-memory test profiles. `TenantScope.require()` resolves the current tenant (falling back to the default organization), writes force the org from it, and `TenantScope.assertAccess(org) → 404` is called on single-resource reads; collection reads filter rows by the current tenant. This isolation now covers all tenant-owned domains: `departments`, `shift_templates`, `rosters`, `user_accounts`, `employees`, `teams`, `leave_requests`, `overtime_requests`, `attendance_sessions`, `attendance_corrections`, `roster_assignments`, `approval_flows`/`approval_actions`, `handovers`, `notifications`, `audit_logs`, `shift_swap_requests`, and `risk_*` tables.
- Background jobs (`OutboxProcessor`, `KafkaRelayer`, `NotificationDeliveryProcessor`) run outside a request context and remain global by design; `RiskAnalysisScheduler` and `NotificationProjector` each derive the tenant from the triggering event's `organization_id` (falling back to the current/default org) so their writes land in the correct tenant even on replay.
- Organization administration is now exposed: `ADMIN`-gated organization CRUD and account provisioning (`POST /api/v1/admin/accounts`), with the authenticated user's current organization served by `GET /api/v1/organizations/current`.
...
Partitioning, archival, soft deletion policy, and defense-in-depth database enforcement. The V4 `organization_id` discriminator column exists on every tenant-owned table, and service-layer `TenantScope` enforcement now covers every domain (see §4). Residual follow-ups: `NotificationDeliveryService.findDue` and the `AttendanceEngine.clockIn` presence probe read stores globally (writes stay tenant-correct; the probes only ever cause spurious conflicts or cross-tenant opportunistic draining by design); the file-based `PayrollCsvIntegration` writes no tenant identifier and is a disabled, unwired reference adapter. Once all reads and writes are scoped, PostgreSQL Row-Level Security policies can be added as a defense-in-depth guard against service-layer omissions.
