...
The `assistant` module answers natural-language questions read-only: `AssistantIntentClassifier` maps the question to topics, `AssistantContextProvider` assembles a grounded snapshot from existing query services, and `HeuristicAssistant` answers offline while `LlmAssistant` uses the same provider configuration as the risk advisor (`workforce.assistant.ai-*`, defaulting to `workforce.risk.ai-*`) with heuristic fallback. LLM calls share a single provider-aware OpenAI-compatible client (`com.workforceos.ai.WorkforceLlmClient`) that accepts `openai`, `muse` (default model `muse`), or `spark` (default model `spark-1.3`) providers, each config-gated behind its own base URL and API key.
...
- Defense-in-depth database enforcement: tenant isolation is enforced today in the service layer via `TenantScope` across every tenant-owned domain; PostgreSQL Row-Level Security would add a persistence-layer backstop. Two residual global reads (`NotificationDeliveryService.findDue`, `AttendanceEngine.clockIn` presence probe) stay deliberately unscoped — writes remain tenant-correct and the probes only ever cause spurious conflicts or opportunistic draining — and the `PayrollCsvIntegration` reference adapter writes no tenant identifier (see [DATABASE-DESIGN.md §9](DATABASE-DESIGN.md)).
...
The Angular frontend implements three guarded pages behind a shared header: the home workforce overview (dashboard KPIs, headcount by department, recent employees), an employee management page (list, create, edit, deactivate, search), and the risk intelligence page. The risk page mirrors the pipeline visually: `RiskPipelineComponent` renders the six pipeline stages, and `RiskDashboardComponent` presents the persisted summary (KPI cards, latest analysis, advisor mode), interactive charts (severity mix, risk by type, risk by department, detections per day), a resolvable alerts table, and an expandable assessments table with recommendations and a run-analysis action. All pages use the same `authGuard`, `AuthService` token handling, and `AuthInterceptor` bearer attachment. UI journeys are exercised end-to-end with Playwright (`frontend/e2e`: login, home, employees, risk, and a journey smoke that saves screenshots to `screenshot/`) — see [TEST-PLAN.md §6.5](TEST-PLAN.md).
